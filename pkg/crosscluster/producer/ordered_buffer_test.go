...
func makeKV(key string, wall int64, value string) *kvpb.RangeFeedValue {
	return makeKVWithPrev(key, wall, value, "")
}

func makeKVWithPrev(key string, wall int64, value string, prevValue string) *kvpb.RangeFeedValue {
	...
	if prevValue != "" {
		kv.PrevValue = roachpb.Value{
			RawBytes: []byte(prevValue),
		}
	}
	return kv
}
...
func getAllEventsFromDisk(
	t *testing.T, b *repstream.OrderedBuffer, ctx context.Context, resolvedTs hlc.Timestamp,
) []kvpb.RangeFeedEvent {
...
func TestFlushToDiskWhenResolvedTsAboveMin(t *testing.T) {
	...
	buf := repstream.NewOrderedBuffer(cfg)
	require.NoError(t, buf.Add(ctx, makeKVWithPrev("k2", 20, "v2", "old_v2")))
	require.NoError(t, buf.Add(ctx, makeKV("k1", 10, "v1")))
	require.NoError(t, buf.AddDelRange(ctx, makeDelRange("k0", "k1", 5)))
	require.NoError(t, buf.Add(ctx, makeKVWithPrev("k1", 20, "v2", "v1")))
	...
	for i := 1; i < len(events); i++ {
		...
		require.Equal(t, ts(exp.wall), events[i].Val.Value.Timestamp, "event %d timestamp", i)
		require.Equal(t, roachpb.Key(exp.key), events[i].Val.Key, "event %d key", i)
		if exp.prevValue != "" {
			require.Equal(t, []byte(exp.prevValue), events[i].Val.PrevValue.RawBytes, "event %d prev value", i)
		} else {
			require.Empty(t, events[i].Val.PrevValue.RawBytes, "event %d prev value should be empty", i)
		}
	}
}
...
func TestOrderedBufferByteSizeFlush(t *testing.T) {
	...
	testEvents := []kvpb.RangeFeedEvent{
		{Val: makeKV("k1", 1, "ab")},
		{Val: makeKVWithPrev("k2", 2, "abc", "old")},
	...
	require.NotNil(t, events[1].Val, "event 1 should be a KV")
	require.Equal(t, []byte("old"), events[1].Val.PrevValue.RawBytes, "prev value should be preserved")
}
...
func TestOrderedBufferRandomizedSequence(t *testing.T) {
	...
	for i, skv := range streamKVs {
		...
		if i%3 == 0 {
			allKVs[i].PrevValue = roachpb.Value{
				RawBytes: []byte(fmt.Sprintf("prev_%d", i)),
			}
		}
	}
	var maxWall int64
	...
	for i := 0; i < numDelRanges; i++ {
		...
		start := roachpb.Key(fmt.Sprintf("/a/delrange/dr%d", i))
		end := start.PrefixEnd()
	...
	seen := make(map[keyTs]bool)
	for _, kv := range allKVs {
		seen[keyTs{string(kv.Key), kv.Value.Timestamp.WallTime}] = true
	}
	uniqueDelRanges := allDelRanges[:0]
	for _, dr := range allDelRanges {
		kt := keyTs{string(dr.Span.Key), dr.Timestamp.WallTime}
		if !seen[kt] {
			seen[kt] = true
			uniqueDelRanges = append(uniqueDelRanges, dr)
		}
	}
	allDelRanges = uniqueDelRanges
	...
	for _, rts := range checkpoints {
		...
		for _, e := range events {
			ets, _ := eventTsAndKey(e)
			require.False(t, ets.Compare(rts) > 0,
				"event ts %s > resolvedTs %s", ets, rts)
		}
		require.True(t, isEventsSorted(events),
	...
	for cpIdx, rts := range checkpoints {
		prevRts := hlc.Timestamp{WallTime: -1}
		if cpIdx > 0 {
		...
		for i := range expected {
			...
			if ex.Val != nil {
				...
				require.Equal(t, ex.Val.PrevValue.RawBytes, g.Val.PrevValue.RawBytes, "checkpoint %d event %d: prev value", cpIdx, i)
			} else {
