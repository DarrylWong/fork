...
var debugClearGossipBootstrapCmd = &cobra.Command{
	Use:   "clear-gossip-bootstrap <store directory>",
...
This is useful when you want a node to start fresh without attempting to
contact previously discovered nodes. The store must be offline (no server running).
`,
	Args: cobra.ExactArgs(1),
...
func runDebugClearGossipBootstrap(cmd *cobra.Command, args []string) error {
	...
	// Read the current bootstrap info to display it
	...
	if !found {
		fmt.Println("No bootstrap info found in store")
		return nil
	}

	fmt.Printf("Found bootstrap info with %d addresses:\n", len(bi.Addresses))
	for i, addr := range bi.Addresses {
		fmt.Printf("  %d: %s\n", i+1, addr.String())
	}
	...
	fmt.Println("\nBootstrap addresses cleared successfully")
	fmt.Println("On next startup, the node will not attempt to contact these addresses")
