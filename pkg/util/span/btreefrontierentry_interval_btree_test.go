...
func newItem(s roachpb.Span) *btreeFrontierEntry {
	i := nilT.New()
	i.SetKey(s.Key)
...
func spanFromItem(i *btreeFrontierEntry) roachpb.Span {
...
func perm(n int) (out []*btreeFrontierEntry) {
...
func rang(m, n int) (out []*btreeFrontierEntry) {
	for i := m; i <= n; i++ {
		out = append(out, newItem(spanWithEnd(i, i+1)))
	}
	return out
...
func all(tr *btree) (out []*btreeFrontierEntry) {
	it := tr.MakeIter()
	it.First()
	for it.Valid() {
...
func TestBTreeSeekOverlapRandom(t *testing.T) {
	...
	for i := 0; i < trials; i++ {
		...
		const count = 1000
		items := make([]*btreeFrontierEntry, count)
		itemSpans := make([]int, count)
		for j := 0; j < count; j++ {
			var item *btreeFrontierEntry
		...
		for j := 0; j < scanTrials; j++ {
			var scanItem *btreeFrontierEntry
...
func TestBTreeCloneConcurrentOperations(t *testing.T) {
	...
	for i, tree := range trees {
		var wantpart []*btreeFrontierEntry
		if i < len(trees)/2 {
