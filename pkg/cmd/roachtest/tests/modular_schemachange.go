...
func runSchemaChangeDuringTPCC(ctx context.Context, t test.Test, c cluster.Cluster, cfg schemaChangeConfig) {
	...
	validationOp := modular.NewOperation("validate schema", operations.ValidateSchema("tpcc"))
...
func createSchemaChangeOperations(database string) []modular.Operation {
	return []modular.Operation{
		...
		modular.NewOperation("add index to order_line",
			addIndexToOrderLine(database)),
		modular.NewOperation("add index to customer",
			addIndexToCustomer(database)),
		modular.NewOperation("add index to warehouse",
			addIndexToWarehouse(database)),

		// Column operations
		modular.NewOperation("add column to customer",
			operations.AddColumn(database, "customer", "c_notes", "TEXT")),
		modular.NewOperation("add column to warehouse",
			operations.AddColumn(database, "warehouse", "w_notes", "TEXT")),
		modular.NewOperation("add column to order_line",
			operations.AddColumn(database, "order_line", "ol_notes", "TEXT")),

		// Table operations
		modular.NewOperation("create audit table",
			operations.CreateTable(database, "audit_log", "id SERIAL PRIMARY KEY, table_name TEXT, operation TEXT, timestamp TIMESTAMP DEFAULT NOW()")),
		modular.NewOperation("create temp table",
			operations.CreateTable(database, "temp_data", "id INT PRIMARY KEY, data TEXT")),

		// Random operations
		modular.NewOperation("random schema change",
			operations.RandomSchemaChange(database)),
		modular.NewOperation("random index creation",
