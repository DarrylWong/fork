...
func runMultiTenantDistSQL(
	...
	inst1Conn, err := c.ConnE(ctx, t.L(), 1, option.TenantName(tenantName), option.AuthMode(install.AuthRootCert))
	require.NoError(t, err)
	...
	for i := 0; i < numInstances; i++ {
		...
		m.Go(func(ctx context.Context) error {
			...
			dbi, err := c.ConnE(ctx, t.L(), node, option.TenantName(tenantName), option.SQLInstance(sqlInstance), option.AuthMode(install.AuthRootCert))
			require.NoError(t, err)
			iter := 0
			for {
