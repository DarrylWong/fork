...
func Toxify(
	...
	if err := func() error {
		if err := c.RunE(ctx, option.WithNodes(c.All()), "curl", "-Lfo", "toxiproxy-server", toxiURL); err != nil {
			return err
		}
		if err := c.RunE(ctx, option.WithNodes(c.All()), "chmod", "+x", "toxiproxy-server"); err != nil {
			return err
		}

		if err := c.RunE(ctx, option.WithNodes(node), "mv cockroach cockroach.real"); err != nil {
			return err
		}
	...
	for _, i := range node {
		...
		toxPort := 8474 + i
		if err := c.RunE(ctx, option.WithNodes(n), fmt.Sprintf("./toxiproxyd %d 2>/dev/null >/dev/null < /dev/null", toxPort)); err != nil {
			return nil, errors.Wrap(err, "toxify")
		}
...
func (tc *ToxiCluster) PoisonedPGAddr(
	...
	urls, err := tc.ExternalPGUrl(ctx, tc.t.L(), node, roachprod.PGURLOptions{})
	if err != nil {
		return nil, err
	}
...
func (tc *ToxiCluster) Measure(ctx context.Context, fromNode int, stmt string) time.Duration {
	...
	result, err := tc.Cluster.RunWithDetailsSingleNode(ctx, tc.t.L(), option.WithNodes(tc.Cluster.Node(fromNode)), "time", "-p", "./cockroach", "sql", "--insecure", "--port", strconv.Itoa(port), "-e", "'"+stmt+"'")
