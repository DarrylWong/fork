	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "github"), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			if d.Cmd == "post" {
				...
				// Roachtest failure messages are formatted the same except it tries to extract
				// the file and line number which we don't have. See: `formatFailure`.
			...
			switch d.Cmd {
			case "add-failure":
				refError := errors.Newf("%s", redact.SafeString(d.CmdArgs[0].Vals[0]))
				...
				if len(d.CmdArgs) == 2 {
					...
					for _, e := range errorTypes {
						...
						case "require-no-error-failed":
							...
							failureMsg := fmt.Sprintf("Received unexpected error:\n%+v", refError)
							refError = errors.New(failureMsg)
						case "lose-error-object":
							// Lose the error object which should make our flake detection fail.
							refError = fmt.Errorf("%s", refError)
...
// This includes markdown formatting, and can be pasted directly into a .md file
// to view what the issue will look like.
