...
// TestDAG tests the DAG generation using data-driven tests.
func TestDAG(t *testing.T) {
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "dag"), func(t *testing.T, d *datadriven.TestData) string {
		switch d.Cmd {
		case "dag":
			// Create the same test setup as in TestPlanner
			mod := NewTest("basic plan", 12345)
			ctrl := gomock.NewController(t)
			mod.WithCreateClusterFn(func() cluster.Cluster {
				mockCluster := cluster.NewMockCluster(ctrl)
