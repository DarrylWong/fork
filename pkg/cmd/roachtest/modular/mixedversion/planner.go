...
func (p *testPlanner) initUpgradeSteps(
	...
	stage := mod.NewStage("init upgrade")
	...
	case SystemOnlyDeployment:
		mod.InStage(stage, preserveDowngradeSystem.impl.Description(), preserveDowngradeSystem)
		return
	case SharedProcessDeployment:
		if virtualClusterRunning {
			mod.InStage(stage, preserveDowngradeSystem.impl.Description(), preserveDowngradeSystem)
			mod.InStage(stage, preserveDowngradeForService(p.tenantName()).impl.Description(), preserveDowngradeForService(p.tenantName()))
		}
		return
	...
	case SeparateProcessDeployment:
		...
		return // nothing to do, separate-process servers don't auto-upgrade
