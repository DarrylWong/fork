...
func AddGrafanaAnnotation(
	ctx context.Context, secure bool, req AddAnnotationRequest,
) (string, error) {
	// Cleanup the host name. The Grafana OpenAPI already appends the scheme
	// and leading slash.
	host, _ := strings.CutPrefix(req.Host, "https://")
...
// AddAnnotationRequest is the fields used to create a Grafana annotation
// post request. Only Host and Text are required fields.
