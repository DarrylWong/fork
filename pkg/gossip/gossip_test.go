...
func (ts *testStorage) ReadBootstrapInfo(info *BootstrapInfo) error {
	*info = ts.bootstrapInfo
	return nil
}

func (ts *testStorage) WriteBootstrapInfo(info *BootstrapInfo) error {
	ts.bootstrapInfo = *info
	ts.written = true
	return nil
}

func (ts *testStorage) isWritten() bool {
	return ts.written
}

func (ts *testStorage) Info() BootstrapInfo {
	return ts.bootstrapInfo
}

// TestClearCachedBootstrapAddresses verifies that ClearBootstrapAddresses
// clears cached bootstrap addresses from persistent storage while preserving
// other gossip data.
func TestClearCachedBootstrapAddresses(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
	stopper := stop.NewStopper()
	defer stopper.Stop(ctx)
	...
	// Create a test storage to verify the bootstrap info is written
	ts := &testStorage{}
	require.NoError(t, g.SetStorage(ts))

	// Add some bootstrap addresses first
	testAddrs := []util.UnresolvedAddr{
		util.MakeUnresolvedAddr("tcp", "127.0.0.1:9000"),
		util.MakeUnresolvedAddr("tcp", "127.0.0.1:9001"),
	}
	...
	// Verify bootstrap addresses were added
	require.Greater(t, len(g.bootstrapInfo.Addresses), 0)
	require.Greater(t, len(g.bootstrapAddrs), 0)

	// Add some regular gossip data (should be preserved)
	require.NoError(t, g.AddInfo("important-key", []byte("important-value"), time.Hour))

	// Clear cached bootstrap addresses
	require.NoError(t, g.ClearBootstrapAddresses(ctx))

	// Verify bootstrap addresses were cleared
	require.Equal(t, 0, len(g.bootstrapInfo.Addresses))
	require.Equal(t, 0, len(g.bootstrapAddrs))
	require.Equal(t, 0, len(g.addressExists))

	// Verify it was written to storage
	require.True(t, ts.isWritten())
	storedInfo := ts.Info()
	require.Equal(t, 0, len(storedInfo.Addresses))

	// Verify regular gossip data is preserved
	_, err := g.GetInfo("important-key")
	require.NoError(t, err)
}
