...
func WaitForBalancedReplicas(ctx context.Context, db *gosql.DB, opts ...RetryConfigFunc) error {
	config := getRetryConfig(opts...)
	timeoutCtx, cancel := maybeSetTimeout(ctx, config)
	defer cancel()
	...
	return config.Do(timeoutCtx, func(ctx context.Context) error {
		...
		if len(unbalanced.storeIDs) == 0 {
			...
			config.logVerbose("all stores have range count within %.2f%% of the mean", threshold*100)
			if consecutiveStableIntervals > 2 {
				return nil
			}
		} else {
			config.logVerbose("unbalanced stores: %v, ranges: %v, avg: %f", unbalanced.storeIDs, unbalanced.rangeCounts, unbalanced.avgRangeCount)
			consecutiveStableIntervals = 0
		}
