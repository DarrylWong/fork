...
// failerStateTransition represents a method that the failer
// will validate against its current state.
type failerStateTransition int

const (
	Setup failerStateTransition = iota
	Inject
...
func (f failerStateTransition) String() string {
	switch f {
	case Setup:
		return "Setup()"
	case Inject:
...
func generateFailerPlan(rng *rand.Rand) []failerStateTransition {
	numSteps := 10000
	plan := make([]failerStateTransition, 0, numSteps)
	for range numSteps {
		plan = append(plan, failerStateTransition(rng.Intn(6)))
	}
	return plan
}
...
func isTransitionLegal(
	f *Failer,
	transition failerStateTransition,
) bool {
	switch transition {
	case Setup:
...
func runTransitionStep(
	...
	// used for testing is the only state used by noopFailure.
	f.setupArgs = args
	f.injectArgs = args
...
func Test_FailerLifecycle(t *testing.T) {
	...
	fr.Register()
	f, err := fr.GetFailer("", NoopFailureName, nilLogger(), ConnectionInfo{})
	...
	var step int
	var transition failerStateTransition
	var testPassed bool
	defer func() {
		if !testPassed {
	...
	for step, transition = range plan {
		...
		if isTransitionLegal(f, transition) {
			if err = runTransitionStep(f, transition, injectedErr); err != nil {
				...
				// it must be because we injected an error.
