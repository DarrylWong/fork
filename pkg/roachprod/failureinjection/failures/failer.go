...
var invalidFailerStateErr = errors.New("failure mode is in invalid state, Cleanup() the failure mode and retry")

// Failer is a wrapper over a FailureMode that enforces that we maintain
// a consistent state in regards to a single failure mode usage. The following
// diagram illustrates the valid states and transitions:
//
//          ┌───────────────┐◄────────────────────────────┐
//     ┌───►│ uninitialized │                             │
//     │    └───────┬───────┴────────────────┐            │
//     │            │                        │            │
// Cleanup()     Setup()               Setup() Error  Cleanup()
//     │            │                        OR           │
//     │            │                 Cleanup() Error     │
//     │            │                        │            │
//     │            │                        │            │
...
func (f *Failer) Cleanup(ctx context.Context, l *logger.Logger) error {
	...
	case uninitialized:
		return errors.New("failure mode is not setup")
	case activeFailure:
		// If there is an active failure injected, recover it first.
