...
// run is a wrapper function for SyncedCluster.Run.
...
func (f *IPTablesPartitionNode) runWithDetails(ctx context.Context, l *logger.Logger, node install.Nodes, args ...string) (install.RunResultDetails, error) {
	if f.c.IsLocal() {
		l.Printf("Local cluster detected, skipping iptables command")
...
sudo iptables %[1]s OUTPUT -s {ip:%[2]d:public} -p tcp --dport {pgport:%[2]d} -j DROP;
`
)
...
func (f *IPTablesPartitionNode) Cleanup(_ context.Context, _ *logger.Logger, _ FailureArgs) error {
	return nil
}

// PacketsDropped returns the number of packets dropped on a given node due to iptables rules.
