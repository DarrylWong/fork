...
func makeGenericFailure(
	clusterName string, l *logger.Logger, failureModeName string, options ClusterOptions,
) (*GenericFailure, error) {
	c, err := roachprod.GetClusterFromCache(l, clusterName, install.SecureOption(options.secure), install.PGUrlCertsDirOption(options.localCertsPath))
	if err != nil {
		return nil, err
	}
