...
func decodeTimestampKey(encoded []byte) (hlc.Timestamp, error) {
	...
	if len(encoded) == 12 {
		return hlc.Timestamp{}, errors.AssertionFailedf(
			"encoded key missing rangefeed key suffix (got %d bytes, expected >12)",
			len(encoded),
		)
	}
	ts := hlc.Timestamp{
...
func deepCopyRangeFeedValue(v *kvpb.RangeFeedValue) *kvpb.RangeFeedValue {
	out := &kvpb.RangeFeedValue{
		Key: append(roachpb.Key(nil), v.Key...),
		Value: roachpb.Value{
			RawBytes:  append([]byte(nil), v.Value.RawBytes...),
			Timestamp: v.Value.Timestamp,
		},
	}
	if len(v.PrevValue.RawBytes) != 0 {
		out.PrevValue = roachpb.Value{
			RawBytes: append([]byte(nil), v.PrevValue.RawBytes...),
		}
	}
	return out
}

func deepCopyRangeFeedDeleteRange(d *kvpb.RangeFeedDeleteRange) *kvpb.RangeFeedDeleteRange {
	return &kvpb.RangeFeedDeleteRange{
		Span: roachpb.Span{
			Key:    append(roachpb.Key(nil), d.Span.Key...),
			EndKey: append(roachpb.Key(nil), d.Span.EndKey...),
		},
...
func (b *OrderedBuffer) Add(ctx context.Context, v *kvpb.RangeFeedValue) error {
	v = deepCopyRangeFeedValue(v)
	ts := v.Value.Timestamp
	return b.addEvent(ctx, ts, &kvpb.RangeFeedEvent{Val: v})
}
...
func (b *OrderedBuffer) AddDelRange(ctx context.Context, d *kvpb.RangeFeedDeleteRange) error {
	d = deepCopyRangeFeedDeleteRange(d)
	ts := d.Timestamp
...
// FlushCount returns the number of times FlushToDisk was called.
func (b *OrderedBuffer) FlushCount() int64 {
	return b.flushCount
}
...
func (b *OrderedBuffer) GetEventsFromDisk(
	...
	if lastKey != nil {
		...
		if retErr = writer.DeleteRange(nil, endExclusive); retErr != nil {
