...
var mtStaticTestDirectoryCmd = &cobra.Command{
	Use:   "static-test-directory",
	Short: "start a static test directory server with HTTP API",
	Long: `Starts a static test directory server intended for testing purposes only.

This server provides a TestStaticDirectoryServer that exposes:
- GRPC interface (port 46258) for SQL proxies to discover tenant pods
- HTTP API (port 46259) to manually manage pod lifecycles
...
HTTP API endpoints:
  ...
  GET  /info               - Get current state of all tenants and pods
`,
...
func runStaticTestDirectory(cmd *cobra.Command, args []string) (returnErr error) {
	...
	log.Ops.Infof(ctx, "Starting static test directory server (GRPC:%d, HTTP:%d)",
		staticDirectoryContext.grpcPort, staticDirectoryContext.httpPort)

	// Create static directory server
	...
	grpcListener, err := net.Listen("tcp", fmt.Sprintf("0.0.0.0:%d", staticDirectoryContext.grpcPort))
	if err != nil {
		return err
	}
	...
	httpListener, err := net.Listen("tcp", fmt.Sprintf("0.0.0.0:%d", staticDirectoryContext.httpPort))
	if err != nil {
		return err
	}
...
func (s *staticDirectoryServer) handleInfo(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}

	state := s.dir.GetState()
