...
func alterBackupPlanHook(
	...
	for _, cmd := range alterBackupStmt.Cmds {
		switch v := cmd.(type) {
		case *tree.AlterBackupAddKMS:
			newKmsFn, err = p.TypeAsStringArray(ctx, tree.Exprs(v.NewKMSURI), "ALTER BACKUP")
			if err != nil {
				return nil, nil, nil, false, err
			}
			oldKmsFn, err = p.TypeAsStringArray(ctx, tree.Exprs(v.OldKMSURI), "ALTER BACKUP")
			if err != nil {
				return nil, nil, nil, false, err
			}
		}
...
func doAlterBackupPlan(
	...
	baseStores, err := p.ExecCfg().DistSQLSrv.ExternalStorageFromURI(ctx, backup, p.User())
	if err != nil {
		return errors.Wrapf(err, "failed to open backup storage location")
	}
	defer baseStores.Close()

	opts, err := readEncryptionOptions(ctx, baseStores)
	if err != nil {
		return err
	}

	ioConf := baseStores.ExternalIOConf()
	...
	if !oldKMSFound {
		return errors.New("no key in old_kms matches the provided backup")
	}
	...
	plaintextDataKey, err = getEncryptionKey(ctx, encryption, baseStores.Settings(),
		baseStores.ExternalIOConf())
	if err != nil {
		return err
	}
	...
	err = writeNewEncryptionInfoToBackup(ctx, encryptionInfo, baseStores, len(opts))
	if err != nil {
		return err
	}
	return nil
