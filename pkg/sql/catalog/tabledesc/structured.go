...
func (desc *Mutable) ensurePrimaryKey() error {
	if len(desc.PrimaryIndex.KeyColumnNames) == 0 && desc.IsPhysicalTable() {
		...
		s := catpb.Expression("unique_rowid()")
...
func (desc *Mutable) Rewrite(rewriter catalog.DescriptorRewriteFn) error {
	...
	if err := descutil.WalkDescIDs(&desc.TableDescriptor, func(id *catid.DescID) error {
	...
	if err := descutil.WalkTypes(&desc.TableDescriptor, func(t *types.T) error {
		return descutil.RewriteTypeOIDs(t, rewriter)
	}); err != nil {
		return err
	}

	if err := descutil.WalkExpressions(&desc.TableDescriptor, func(expr *catpb.Expression) error {
		if *expr == "" {
			return nil
		}
		newExpr, err := descutil.RewriteExprIDs(string(*expr), rewriter)
		if err != nil {
			return err
		}
	...
	if err := descutil.WalkStatements(&desc.TableDescriptor, func(stmt *catpb.Statement) error {
		if *stmt == "" {
			return nil
		}
		newStmt, err := descutil.RewriteViewQueryIDs(string(*stmt), rewriter)
		if err != nil {
			return err
		}
		*stmt = catpb.Statement(newStmt)
		return nil
	}); err != nil {
		return err
	}

	if err := descutil.WalkRoutineBodies(&desc.TableDescriptor, func(body *catpb.RoutineBody) error {
		if *body == "" {
			return nil
		}
		newBody, err := descutil.RewritePLpgSQLBodyIDs(string(*body), rewriter)
		if err != nil {
			return err
		}
		*body = catpb.RoutineBody(newBody)
		return nil
	}); err != nil {
		return err
	}
