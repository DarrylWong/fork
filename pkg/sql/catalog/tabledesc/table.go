...
func MakeColumnDefDescs(
	...
	if d.HasDefaultExpr() {
		...
		if ret.DefaultExpr != tree.DNull {
			...
			s := descpb.Expression(tree.Serialize(d.DefaultExpr.Expr))
			col.DefaultExpr = &s
		}
	}
	...
	if d.HasOnUpdateExpr() {
		...
		s := descpb.Expression(tree.Serialize(d.OnUpdateExpr.Expr))
		col.OnUpdateExpr = &s
	}
	...
	if d.IsComputed() {
		...
		s := descpb.Expression(tree.Serialize(d.Computed.Expr))
		col.ComputeExpr = &s
	}
