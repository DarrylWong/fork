...
func datadrivenWorker(
	...
	for c := 0; c < chains; c++ {
		...
		ext := &chainExtended{
			rng: rng, sorted: sorted, sub: sub, dropped: dropped, pks: pks,
		}
...
func formatPKs(sorted []*Table, pks PKAssignment) string {
	parts := make([]string, 0, len(sorted))
	for _, t := range sorted {
		parts = append(parts, fmt.Sprintf("%s=%s", t.Name, formatArgs(pks[t.Name])))
	}
	return strings.Join(parts, " ")
}
...
func parseRunArgs(t *testing.T, td *datadriven.TestData) runArgs {
	...
	for _, arg := range td.CmdArgs {
		...
		case "mix":
			...
			require.Len(t, parts, 2, "mix must be UPDATE/DELETE")
			up, err := strconv.Atoi(parts[0])
			require.NoError(t, err)
			d, err := strconv.Atoi(parts[1])
			require.NoError(t, err)
			args.mix = OpMix{Update: up, Delete: d}
		default:
